...
DIR_SUFFIX = datetime.now().strftime("%d%m%Y")
INFOS_DIR = os.path.abspath(os.path.join("..", "infos", DIR_SUFFIX))
CONSOLIDADO_DIR = os.path.abspath(os.path.join("..", "consolidado", DIR_SUFFIX))
...
print("\n" + "=" * 60)
print("Movendo arquivos .txt para", INFOS_DIR)
for txt in glob("*.txt"):
    try:
        dest = os.path.join(INFOS_DIR, os.path.basename(txt))
        shutil.move(txt, dest)
        print("Movido:", txt, "->", dest)
    except Exception as e:
        print("Erro ao mover", txt, ":", e)

print("\nMovendo arquivos .csv para", CONSOLIDADO_DIR)
for csv in glob("*.csv"):
    try:
        dest = os.path.join(CONSOLIDADO_DIR, os.path.basename(csv))
