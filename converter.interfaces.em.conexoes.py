...
def process_file(filepath):
    ...
    output_detailed = filepath.replace('.interfaces_all.csv', '.conexoes.csv')
    ...
    output_sum = filepath.replace('.interfaces_all.csv', '.conexoes.SUM.csv')
    df_sum.to_csv(output_sum, sep=';', index=False)
    print(f" -> Gerado: {output_sum} ({len(df_sum)} linhas)")

def main():
    files = glob.glob('*.interfaces_all.csv')
    
    if not files:
        print("Nenhum arquivo *.interfaces_all.csv encontrado.")
        return
